    ...
    def generate(self, data):
        ...
        with torch.no_grad():
            ...
            for i in range(len(u)):
                ...
                # Collect items and relevance scores for MMR
                all_items.extend(candidate_ids)
                for idx, cid in enumerate(candidate_ids):
                    relevance_scores[cid] = relevance_scores.get(cid, 0) + torch.matmul(log_emb[i], self.recsys.model.item_emb(torch.LongTensor([cid]).to(self.device)).T).item()
        
        selected_items = calculate_mmr(all_items, relevance_scores, self.args.lambda_val)
